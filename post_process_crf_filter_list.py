...
def box_method(image_path, seg_path, save_path, image_name, filter_list):
    #print(image_name)
    image, bb, w, h = open_yolo_sort(image_path, image_name)
    ...
    if bb is not None:       
        ...
        # A: this method will also get rid of any false positives from DeepLab.
            # FOR THE FILTER LIST. We want to only process the classes that are allowed
            if label in filter_list:
                ...
                # problem classes: [2, 4, 6, 15] (needs to be smaller) (chaves fechadas, fusivel, tripolar)
...
def crf_method(image_path, seg_image, image_name, filter_list):
    image, bb, w, h = open_yolo_sort(image_path, image_name)
...
@stopit_after_timeout(300, raise_exception=True)
def post_process(image_path, seg_path, save_path, image_name):
    # the functions that use filter list at the moment are set to
    # "in" filter list. This is excluding all except (religador)
    filter_list = [10]
    try:
        #print(image_name)
        image, seg_image, cut_image = box_method(image_path, seg_path, save_path, image_name, filter_list)
        colors, labels = np.unique(cut_image, return_inverse=True)
        n_labels = len(set(labels.flat))
        if (n_labels > 1):
            crf_image = crf_method(image_path, cut_image, image_name, filter_list)
...
# https://stackoverflow.com/questions/66843324/run-one-function-on-different-cpus
import sys

def post_process_concurrent(image_list):
    for image in image_list:
        part_post_process(image)
...
def main():
    ...
    save_list = os.path.join(os.path.join(opt.save_path, "*.*"))
    # using glob to get the image names, since they can have more than one extension
    file_list = glob.glob(file_list)
    save_list = glob.glob(save_list)
    image_list = []
    for file in file_list:
        f = file.split("/")[-1]
        if ".txt" not in file and opt.save_path+(os.path.splitext(f)[0]+".png") not in save_list:
            file = file.split("/")[-1]
            #file = os.path.splitext(file)[0]
            image_list.append(file)
    split_image_list = np.array_split(image_list, 15)
    try:
        concex.concurrent_process_execution(post_process_concurrent, [split_list for split_list in split_image_list])
        #p_umap(part_post_process, image_list, num_cpus = 15)
